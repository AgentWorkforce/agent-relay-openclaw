 ...
 *   3. the driver-managed spawn bridge (`bridge/spawn-from-env.mjs`) as PID 1
 *
...
export class DockerSpawnProvider implements SpawnProvider {
   ...
   * It runs runtime-setup via the package CLI, starts the gateway, then hands off to the
   * driver-managed spawn bridge.
   */
  private buildEntrypointScript(gatewayPort: number): string[] {
    // Shell script that runs setup, starts gateway, waits for health, then execs the spawn bridge.
    // Uses sh -c so it works in minimal alpine images.
    // Runtime setup via package CLI, then gateway, then the driver-managed spawn-from-env bridge,
    // which starts a broker + spawns the agent via PTY and waits for exit.
    const script = [
      'set -e',
      // Runtime setup: auth conversion, openclaw.json, identity files, dist patching
      'npx @agent-relay/openclaw runtime-setup',
      // Resolve bridge.mjs + spawn-from-env.mjs from the installed package and symlink to known paths.
      ...
      `  sleep 1`,
      `done`,
      // Driver-managed spawn: reads AGENT_NAME, AGENT_CLI, RELAY_WORKSPACE_KEY etc. from env,
      // starts a broker, spawns the agent via PTY, and waits for exit as PID 1.
